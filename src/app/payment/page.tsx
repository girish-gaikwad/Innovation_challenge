...
const PaymentSuccess = ({ isOpen, onClose }: PaymentSuccessProps) => {
      ...
      <div className="rounded-2xl p-8 flex flex-col items-center transform animate-fadeIn" style={{ backgroundColor: '#fdf5e6' }}>
        <div className="w-16 h-16 relative mb-4">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div 
              className="w-4 h-8 transform rotate-45 translate-x-[10%] translate-y-[-10%]"
              style={{ borderRight: '4px solid #8b4513', borderBottom: '4px solid #8b4513' }}
            />
          </div>
        </div>
...
const PaymentScreen = () => {
  ...
  const validateForm = () => {
    const { cardHolderName, cardNumber, cvv } = formData;
    if (!cardHolderName || !cardNumber || !cvv || !selectedCard) {
      ...
      <div className="px-8">
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-5 gap-8">
            {cardImages.map((card) => (
              <div
                key={card.id}
                className={`aspect-square rounded-lg flex items-center justify-center cursor-pointer shadow-md transition-all duration-300 hover:shadow-lg ${
                  selectedCard === card.id ? 'bg-opacity-20' : 'bg-opacity-10'
                }`}
                style={{
        ...
        <form onSubmit={handleSubmit} className="mt-12">
          ...
          <div className="space-y-4">
            {[
              { label: "Card Holder Name", name: "cardHolderName" as const },
              { label: "Card Number", name: "cardNumber" as const, placeholder: "Enter your card number" }
            ].map((field) => (
              <div key={field.name}>
                <label htmlFor={field.name} className="flex items-center text-sm font-medium mb-1" style={{ color: '#8b4513' }}>
            ...
            <div className="flex justify-center space-x-4 mt-4">
              <div className="flex-1">
                <label htmlFor="expirationDate" className="flex items-center text-sm font-medium mb-1" style={{ color: '#8b4513' }}>
                  <span className="text-red-500 mr-1">*</span>
                  Expiration Date
                </label>
                <DatePicker
  ...
  className="w-full p-3 rounded-lg outline-none react-datepicker"
  dateFormat="MM/yyyy"
  showMonthYearPicker
  required
/>
              ...
              <div className="flex-1">
                <label htmlFor="cvv" className="flex items-center text-sm font-medium mb-1" style={{ color: '#8b4513' }}>
                  <span className="text-red-500 mr-1">*</span>
                  CVV
                </label>
                <input
          ...
          <div className="flex justify-end mt-8 py-4" style={{ borderTop: '1px solid #8b4513' }}>
            <button
              type="submit"
